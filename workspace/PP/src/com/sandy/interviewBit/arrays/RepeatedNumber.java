...
public class RepeatedNumber {
	public int repeatedNumber(final List<Integer> A) {
		...
		int totalNums = A.size()-1;
		double root = Math.sqrt(totalNums);
		int bucketCount = (int) Math.floor(root);
		int bucketSize = (int) Math.ceil(root);
		int lastBucketSize = 0;
		int buckets[];
		
		if(bucketCount*bucketSize > totalNums) {
			while (bucketCount*bucketSize > totalNums) {
				bucketSize--;
			}
			int remaining = totalNums - (bucketSize*bucketCount);
			bucketCount++;
			lastBucketSize = remaining;
		} else if(bucketCount*bucketSize < totalNums){
			int remaining = totalNums - (bucketSize*bucketCount);
			bucketCount++;
			lastBucketSize = remaining;
		} else {
		...
		int targetBucket = -1;
		for(int curr: A) {
			int currBucket = (curr-1)/bucketSize;
			buckets[currBucket]++;
			if(currBucket == (buckets.length - 1)) {
				if(buckets[currBucket] > lastBucketSize) {
					targetBucket = currBucket;
					break;
				} 
			}
				
			else if(buckets[currBucket] > bucketSize) {
					targetBucket = currBucket;
					break;
				}
		...
		int result = -1;
		int leftRange = targetBucket*bucketSize + 1;
		int rightRange = leftRange + bucketSize -1;
		if(rightRange > totalNums) {
			rightRange = totalNums;
		}
		HashMap<Integer, Integer> checkMap = new HashMap<Integer, Integer>();
		
		for(int curr: A) {
			if(curr >= leftRange && curr<= rightRange) {
...
//		al.add(1);
		...
		System.out.println(repeatedNumber.repeatedNumber(al));
	}
}
